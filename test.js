...
function fetchByIds(ids) {
  ...
  return ids.map(x=> {return [{id: x, value: 3*x}, {id: x, value: 2*x}]}).reduce((a,b)=>a.concat(b));
}

const cachedFetchById = cache.cacheById(1000, fetchById);
const cachedFetchByIds = cache.cachePerId(1000, fetchByIds);
